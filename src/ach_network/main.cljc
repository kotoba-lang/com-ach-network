(ns ach_network.main
  "Kotodama WASM entrypoint for the Ach_network clean-room actor (L4) — Clojure port.
  ...
  facts (`ach_network.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "ach_network")
(def tier "L4")
...
(defn as-int-kernel
  ...
  of ach_network.main/as-int stays here as the retained oracle."
  [v]
  (if (pos? v) v 0))
...
(defn emit-facts
  "EAVT facts for one record: {\"ach_network.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ach_network-compat" :tier tier :entities entities} 200])
...
(defn start! [] :ach_network-compat/ready)
